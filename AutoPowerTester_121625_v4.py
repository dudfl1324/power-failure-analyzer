...
RESULTS_HEADER_FONT_SIZE = 12 #only int, any decimals will be truncated
RESULTS_BODY_FONT_SIZE = 24 #only int, any decimals will be truncated
...
def open_main_window(root: tk.Tk) -> None:
    ...
    # ----- Heading font -----
    try:
        # Reuse named font if it already exists in this process, then reconfigure it
        heading_font = tkfont.Font(name="ResultsHeadingFont", exists=True)
    except Exception:
        heading_font = tkfont.Font(font="TkHeadingFont")
    heading_font.configure(size=int(RESULTS_HEADER_FONT_SIZE), weight="bold")

    # Apply to BOTH base and custom heading styles (Windows themes often honor the base)
    ...
    # IMPORTANT: set tag fonts explicitly so the row font always follows RESULTS_BODY_FONT_SIZE
    tree.tag_configure("green_row", background="#b6fcd5", font=body_font)
    tree.tag_configure("red_row", background="#ffb6b6", font=body_font)
