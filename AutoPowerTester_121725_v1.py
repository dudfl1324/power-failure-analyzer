...
def load_config() -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, str], List[Dict[str, str]]]:
    ...
    if "POWER_SUPPLIES" in data:
        ...
        addr = data.get("POWER_SUPPLY_ADDRESS", "GPIB0::6::INSTR")
        ps_list = [{"name": "PSU1", "address": addr}]

    if not ps_list:
        ps_list = [{"name": "PSU1", "address": "GPIB0::6::INSTR"}]
...
def show_log_file_open_error(parent: tk.Misc, path: str) -> None:
    ...
    tk.Label(container, text="LOG FILE IS OPEN", font=("TkDefaultFont", 18, "bold"), fg="red").pack(
        pady=(0, 10)
    )
    tk.Label(
        container,
...
def get_pf_status(model: str, avg_current: float, samples: Optional[List[float]] = None) -> str:
    ...
    W748_thr = criteria.get("W748")
    W74A_thr = criteria.get("W74A")

    if W748_thr is not None and W74A_thr is None:
        if avg_current >= pass_thr:
            return "PASS"
        if avg_current >= W748_thr:
            return "FAIL(W748)"
        return "FAIL(W74A)"

    if W748_thr is None and W74A_thr is not None:
        if avg_current >= pass_thr:
            return "PASS"
        if avg_current >= W74A_thr:
            return "FAIL(W74A)"
        return "FAIL(W748)"
    
    if W748_thr is not None and W74A_thr is not None:
        if W748_thr > W74A_thr:
            if avg_current >= pass_thr:
                return "PASS"
            if avg_current >= W748_thr:
                return "FAIL(W748)"
            return "FAIL(W74A)"
        if W74A_thr > W748_thr:
            if avg_current >= pass_thr:
                return "PASS"
            if avg_current >= W74A_thr:
                return "FAIL(W74A)"
...
def prompt_no_current_detected(parent: tk.Misc, supply_name: str, question_en: str, anchor_widget=None) -> bool:
    ...
    tk.Label(container, text=f"PSU: {supply_name}", font=("TkDefaultFont", 14, "bold"), fg="red").pack(
        anchor="w", pady=(4, 8)
    )

    tk.Label(container, text=question_en, font=("TkDefaultFont", 13), wraplength=620, justify="left").pack(
        anchor="w", pady=(0, 6)
    )
    tk.Label(container, text=question_es, font=("TkDefaultFont", 13), wraplength=620, justify="left").pack(
        anchor="w", pady=(0, 10)
    )
    ...
    tk.Button(btn_row, text="YES / SÍ", font=("TkDefaultFont", 13, "bold"), width=10, command=on_yes).pack(
        side="left", padx=10
    )
    tk.Button(btn_row, text="NO", font=("TkDefaultFont", 13, "bold"), width=10, command=on_no).pack(
        side="left", padx=10
    )
...
def _build_power_supplies_tab(tab: tk.Frame) -> None:
    ...
    tk.Button(btns, text="Add / Update", font=("TkDefaultFont", 11), command=add_or_update_supply).pack(
        side="left", padx=5
    )
...
def _build_workers_tab(tab: tk.Frame) -> None:
    ...
    tk.Button(wb_frame, text="Add / Update", font=("TkDefaultFont", 11), command=add_or_update_worker).pack(
        side="left", padx=5
    )
