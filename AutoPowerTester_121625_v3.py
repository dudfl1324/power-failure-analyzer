...
# Power supply defaults (single source of truth)
...
def _normalize_supply_entry(entry: Any, idx: int) -> Dict[str, str]:
    """Normalize a single power supply entry to a safe shape."""
    # Default naming: PSU1..PSU4
...
def _normalize_power_supplies(ps_list: Any) -> List[Dict[str, str]]:
    """Return a normalized list (max 4) of power supplies with {name,address} keys.

    - Never returns an empty list; will return a single PSU fallback.
    - Keeps empty addresses as empty strings (so we can warn loudly later).
    """
...
def _warn_if_missing_psu_addresses(ps_list: List[Dict[str, str]]) -> None:
    """Warn the operator/admin if any PSU address is missing or empty.

    This does not stop the program; it simply explains why connection may fail.
    """
    missing = [ps.get("name", "PSU") for ps in ps_list if not str(ps.get("address", "")).strip()]
    if not missing:
        return

    # Show a warning early so the issue is obvious (especially on new installations).
...
def load_config() -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, str], List[Dict[str, str]]]:
    ...
    # If it somehow ended up empty, still ensure a safe fallback.
    if not ps_list:
...
# Warn early if PSU addresses are missing/empty, so it doesn't look like a mysterious VISA bug later.
try:
    # Ensure a root exists for messagebox (but don't create the main UI yet)
    _tmp = tk.Tk()
    _tmp.withdraw()
    _warn_if_missing_psu_addresses(POWER_SUPPLIES)
    _tmp.destroy()
except Exception:
    # Don't block startup if warning UI fails for some reason
    pass
...
def get_pf_status(model: str, avg_current: float, samples: Optional[List[float]] = None) -> str:
    """Return PASS/FAIL code based on model rules and sample behavior."""
...
def _run_real_measurement(
    ...
    try:
        ...
        # Improve connection error message clarity for operators
...
def _build_power_supplies_tab(tab: tk.Frame) -> None:
    ...
    def add_or_update_supply() -> None:
        ...
        # Allow empty address but warn strongly (operators may use pseudo mode, etc.).
        if not ad:
...
def open_main_window(root: tk.Tk) -> None:
    ...
    def run_from_panel(
        ...
        # New validation: warn if PSU address is missing when attempting real measurement
                ...
                while True:
                    ...
                    if kind == "status":
                        ...
                        try:
                            ...
                            row = (imei, model, avg_current_str, pf_status, id_val, supply_used, date_val)
                            item_id = tree.insert("", tk.END, values=row, tags=("red_row",))
                            ...
                            progress_var.set("Done: FAIL(Sub PBA)")
                            status_label.config(text="FAIL(Sub PBA)", fg="red")
